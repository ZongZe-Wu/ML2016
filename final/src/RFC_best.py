...
type_list=['normal.']
label_list=[0]
with open(sys.argv[1]+'training_attack_types.txt','r') as attack_type:
...
train_set_input=csv.reader(open(sys.argv[1]+'train','rb'))
...
RFC = RandomForestClassifier(n_estimators=25,verbose=1,max_features=27,n_jobs=-1,random_state=20170111)
RFC.fit(DATA[:,0:41],DATA[:,41].astype(int))
DATA=[]
...
i=0
test_set=[]
test_file=csv.reader(open(sys.argv[1]+'test.in','rb'))
...
prediction=RFC.predict(test_set)
...
count = np.zeros(5)
i=1
prediction_file=open (sys.argv[2],'w')
