...
ET_classfier = ensemble.ExtraTreesClassifier(n_estimators=200,verbose=1,random_state=12304,bootstrap=False,n_jobs=-1)
#adaboost_classfier = AdaBoostClassifier(ET_classfier,n_estimators=50,learning_rate=1)
#adaboost_classfier.fit(DATA[:,0:41],DATA[:,41].astype(int))
ET_classfier.fit(DATA[:,0:41],DATA[:,41].astype(int))
DATA=[]
...
prediction=ET_classfier.predict(test_set)#adaboost_classfier.predict(test_set)#
...
for label in prediction:
    ...
    count[label] += 1
    i+=1
print 'predict distribution',count
