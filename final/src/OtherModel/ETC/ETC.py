...
for row in train_set_input:
    ...
    if row[41] not in label4:
        label4.append(row[41])
...
ETC = ExtraTreesClassifier(n_estimators=30,verbose=1)
ETC.fit(DATA[:,0:41],DATA[:,41].astype(int))
DATA=[]
...
prediction=ETC.predict(test_set)
...
count = np.zeros(5)
i=1
prediction_file=open ('prediction0108_1.csv','w')
