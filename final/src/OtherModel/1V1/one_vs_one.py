...
def main():
  ...
  for i in range(len(data_X)):
    for j in range(i+1,len(data_X)):   
      ...
      [train_positive,train_negative,ET_classfier,feature_select_mask]=ReduceClassesFeature(i,j,data_X[i],data_X[j],importance_treshold)      
      print '[main] create a binary model for',i,',',j
      print 'size',np.shape(train_positive),np.shape(train_negative)
      ...
      one_to_one_list.append([i,j,ET_classfier,model])
      #output the models to file
      model.save('model'+str(i)+str(j)+'.h5')
      feature_reduce_list.append(feature_select_mask.astype(float))
  data_X=[]
  feature_reduce_list = np.asarray(feature_reduce_list)
  np.savetxt('feature_reduce.out',feature_reduce_list,delimiter=',')
  #testing
  print '---testing---'
  ...
  test_score = np.zeros((len(test_set),5)) 
  #pass to each model
  
  for it in range(len(one_to_one_list)):
    model=[]
    [i,j,ET_classfier,model] = one_to_one_list[it]
    #transform by ETC
    importance = ET_classfier.feature_importances_
    feature_select_mask = (importance>importance_treshold)
    test_X = test_set[:,feature_select_mask]#we need the same test set for all different model,so copy the data
    #get the score
    class_prob = model.predict(test_X)#n*1 not n only
    class_prob = np.asarray(class_prob).reshape(-1)#n array here
    print 'model output shape',np.shape(class_prob)
    print 'score shape ',np.shape(test_score[:,i])
    #save the score
    test_score[:,i] += class_prob
    test_score[:,j] += (1-class_prob) 
